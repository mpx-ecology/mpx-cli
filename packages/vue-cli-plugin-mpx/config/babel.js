...
function resolveBabelConfig (api, options, config, target) {
  const cliServicePath = path.dirname(require.resolve('@vue/cli-service'))
  const mpxServicePath = path.dirname(require.resolve('@mpxjs/mpx-cli-service'))
  ...
  const testPath = api.resolve('test')
  ...
  const jsRule = config.module
    ...
    .exclude.add((filepath) => {
      ...
      if (filepath.startsWith(cliServicePath) || filepath.startsWith(mpxServicePath)) {
        return SHOULD_SKIP
      }

      if (filepath.startsWith(testPath) || filepath.startsWith(srcPath)) {
        return SHOULD_TRANSPILE
      }

      if (/\.mpx\.js/.test(filepath)) {
        return SHOULD_TRANSPILE
      }
...
module.exports.resolveBabelConfig = resolveBabelConfig
